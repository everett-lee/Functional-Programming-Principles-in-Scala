...
object Anagrams extends AnagramsInterface {
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    def subtractPairs(acc: Map[Char, Int], pair: (Char, Int)): Map[Char, Int] = {
      val (letter, count) = pair

      if (acc.contains(letter)) {
        acc + (letter -> (acc(letter) - count))
      } else acc
    }

    val folded = (y.toMap foldLeft x.toMap) (subtractPairs) filter (pair => pair._2 > 0)
    (folded toList) sorted
  }
    ...
    def helper(occurrences: Occurrences): List[Sentence] = {
      if (occurrences.isEmpty) List(List())
        ...
        for {
          ...
          restSentence <- helper(subtract(occurrences, comb))
