...
class RecFunSuite {
  ...
  @Test def `empty passes`: Unit =
    assert(balance(List()))

  @Test def `single left bracket fails`: Unit =
    assert(!balance("(".toList))

  @Test def `single right bracket fails`: Unit =
    assert(!balance(")".toList))
  ...
  @Test def `countChange: empty list`: Unit =
    assertEquals(0, countChange(60,List()))

  @Test def `countChange: no money`: Unit =
    assertEquals(0, countChange(0,List(500,5,50,100,20,200,10)))
  ...
  @Test def `pascal: col=3,row=4`: Unit =
